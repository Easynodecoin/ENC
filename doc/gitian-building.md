...
*Setup instructions for a Gitian build of Easynodecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Easynodecoin
...
- [Building Easynodecoin Core](#building-dash-core)
...
Clone the git repositories for Easynodecoin Core and Gitian.
...
in the Easynodecoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build Easynodecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Easynodecoin Core repository.
